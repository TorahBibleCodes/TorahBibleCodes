## BEGIN FUNCTION () #90 - CONVERT EACH LETTER IN ELS SEARCH QUERY TO REGULAR EXPRESSIONS (REGEX)

def fn_ConvertELSQueryToRegex(ListOfSearchTerms):

    ## TEST PRINT OUTPUT
    print("\n")  ## PRINT SPACE
    print("WITHIN FUNCTION:  BEGIN FUNCTION () #90 - CONVERT EACH LETTER IN ELS SEARCH QUERY TO REGULAR EXPRESSIONS (REGEX)")
    ...
    ## DECLARE VARIABLES
    ListOfRegex4ELSSearchTerms = [] ## EMPTY LIST TO HOLD [EACH ELS SEARCH TERM AS INDIVIDUAL LETTERS]
    ...
    for EachWord in ListOfSearchTerms:
        ...
        ListOfRegex4ELSSearchTerms.append(ListOfRegexStrings)

    ## TEST PRINT OUTPUT
    print("\n")  ## PRINT SPACE
    print(f"ListOfRegex4ELSSearchTerms = {ListOfRegex4ELSSearchTerms}")

    ## END FOR LOOP - FOR EACH WORD IN LIST OF SEARCH TERMS
    
    ## TEST PRINT OUTPUT
    print("\n")  ## PRINT SPACE
    print("WITHIN FUNCTION:  END FUNCTION () #90 - CONVERT EACH LETTER IN ELS SEARCH QUERY TO REGULAR EXPRESSIONS (REGEX)")
    
    ## RETURN VARIABLES
    ## return(ListOfRegexStrings)
    return(ListOfRegex4ELSSearchTerms) ## RETURNS LIST OF LISTS OF LETTERS
